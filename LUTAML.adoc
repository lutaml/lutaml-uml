...
association name {
  ...
  from|to association_name[#attribute_name] [{property_string}][cardinality]
}
----

where:

* `type` - association type(association|composition|aggregation|generalization|uses)
* `from` - direction of association, start of the association arrow(for generalization, uses types)
* `to` - direction of association, end of the association arrow(for generalization, uses types)
